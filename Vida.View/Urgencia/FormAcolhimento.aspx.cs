...
using ViverMais.DAO.AtendimentoMedico;
...
{
    public partial class FormAcolhimento : PageViverMais
    {
                        ...
                        if (usuarioprofissional.UnidadeVinculo == usuario.Unidade.CNES)
                        {
                            ...
                            if (iCbo.CBOPertenceEnfermeiro<CBO>(cbo) || iCbo.CBOPertenceMedico<CBO>(cbo))
                            {
                                ...
                                if (Request["codigo"] != null && long.TryParse(Request["codigo"].ToString(), out co_prontuario))
                                {
                                    ...
                                    IList<ViverMais.Model.SituacaoAtendimento> situacoes = iUrgencia.ListarTodos<ViverMais.Model.SituacaoAtendimento>().Where(p => p.Codigo == SituacaoAtendimento.EVASAO || p.Codigo == SituacaoAtendimento.AGUARDANDO_ATENDIMENTO).ToList();
