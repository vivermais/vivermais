...
namespace ViverMais.DAO.Vacina.Movimentacao
{
    public class EstoqueVacinaDAO: VacinaServiceFacadeDAO, IEstoqueVacina
    {
        IList<T> IEstoqueVacina.BuscarPorSalaVacina<T>(int co_salavacina)
        {
            ...
            hql = "FROM ViverMais.Model.EstoqueVacina AS e WHERE e.Sala.Codigo = " + co_salavacina;
        ...
        void IEstoqueVacina.SalvarMovimentacao<M, I>(M _movimento, IList<I> _itens, int co_usuario)
        {
            using (Session.BeginTransaction())
            {
                try
                {
                    long numeromovimentacao = Session.CreateQuery("SELECT MAX(m.Numero) FROM ViverMais.Model.MovimentoVacina m").UniqueResult<long>();
        ...
        {
            string hql = string.Empty;
            hql = "FROM ViverMais.Model.EstoqueVacina AS e WHERE e.Sala.Codigo = " + co_sala;
