...
namespace ViverMais.DAO.AtendimentoMedico.Urgencia
{
    public class VagaUrgenciaDAO : UrgenciaServiceFacadeDAO, IVagaUrgencia
    {
        DataTable IVagaUrgencia.QuadroVagas(string co_unidade, bool formato_linha)
        {
            ...
            IList<ViverMais.Model.VagaUrgencia> vagas = Factory.GetInstance<IVagaUrgencia>().BuscarPorUnidade<VagaUrgencia>(co_unidade);

            DataRow linha = tabela.NewRow();
            linha["Unidade"] = co_unidade.ToString();

            IList<ViverMais.Model.VagaUrgencia> vagasfeminina = vagas.Where(v => v.TipoVaga == VagaUrgencia.FEMININA).ToList();
            ...
            IList<ViverMais.Model.VagaUrgencia> vagasmasculina = vagas.Where(v => v.TipoVaga == VagaUrgencia.MASCULINA).ToList();
            ...
            IList<ViverMais.Model.VagaUrgencia> vagasinfantis = vagas.Where(v => v.TipoVaga == VagaUrgencia.INFANTIL).ToList();
        ...
        {
            IList<ViverMais.Model.VagaUrgencia> vagas = new List<ViverMais.Model.VagaUrgencia>();

            for (int i = 0; i < qtd_feminina; i++)
            {
                ViverMais.Model.VagaUrgencia v = new ViverMais.Model.VagaUrgencia();
        ...
        {
            IList<ViverMais.Model.VagaUrgencia> vagas = (IList<ViverMais.Model.VagaUrgencia>)(object)_vagas;
            IList<ViverMais.Model.VagaUrgencia> vagasatuais = (IList<ViverMais.Model.VagaUrgencia>)(object)_vagasatuais;
            IList<ViverMais.Model.VagaUrgencia> vagastemp = null;
            int diff;
        ...
        {
            ViverMais.Model.VagaUrgencia vaga = Session.CreateQuery("FROM ViverMais.Model.VagaUrgencia v WHERE v.CodigoUnidade='" + co_unidade + "' AND v.TipoVaga='" + tipovaga + "' AND v.Prontuario is null").List<VagaUrgencia>().FirstOrDefault();
            //Factory.GetInstance<IVagaUrgencia>().ListarTodos<ViverMais.Model.VagaUrgencia>().Where(p => p.CodigoUnidade == co_unidade && p.TipoVaga == tipovaga && p.Prontuario == null).FirstOrDefault();
        ...
        T IVagaUrgencia.BuscarPorProntuario<T>(long co_prontuario)
        {
            ...
            hql = "FROM ViverMais.Model.VagaUrgencia AS v WHERE v.Prontuario.Codigo = " + co_prontuario;
        ...
        {
            string hql = string.Empty;
            hql = "FROM ViverMais.Model.VagaUrgencia AS v WHERE v.CodigoUnidade='" + co_unidade + "'";
        ...
        {
            string hql = string.Empty;
            hql = "FROM ViverMais.Model.EspecialidadeAtendimentoUrgence AS e WHERE e.Unidade.CNES='" + co_unidade + "' ORDER BY e.Especialidade.Nome";
            return Session.CreateQuery(hql).List<T>();
        }
